"""
Script calculates citywide block access scores from Current Year data.
06/15/16
Rob Siwiec
"""
...
# Variables
# files
blocks = r"C:\Users\merinor\Documents\ArcGIS\Projects\EBPA_TestRun_2\LOS_BASELINE.gdb\BLOCKS_2013"
parks = r"C:\Users\merinor\Documents\ArcGIS\Projects\EBPA_TestRun_2\LOS_BASELINE.gdb\p_los_baseline"
mile_routes = r"C:\Users\merinor\Documents\ArcGIS\Projects\EBPA_TestRun_2\LOS_BASELINE.gdb\MileRoutes_LOS_Baseline"
closest_routes = r"C:\Users\merinor\Documents\ArcGIS\Projects\EBPA_TestRun_2\LOS_BASELINE.gdb\ClosestRoute_LOS_Baseline"
blocks_current_year = "BLOCKS_2017"
parks_current_year = "p_los_current"
mile_routes_current_year = "MileRoutes_LOS_Current"
closest_routes_current_year = "ClosestRoute_LOS_Current"
block_group_current_year = "BLOCKGROUP_2017"

# layers
blocks_lyr = str(blocks) + "_lyr"
parks_lyr = str(parks) + "_lyr"
mile_routes_lyr = str(mile_routes) + "_lyr"
closest_routes_lyr = str(closest_routes) + "_lyr"
blocks_current_year_lyr = str(blocks_current_year) + "_lyr"
parks_current_year_lyr = str(parks_current_year) + "_lyr"
mile_routes_current_year_lyr = str(mile_routes_current_year) + "_lyr"
closest_routes_current_year_lyr = str(closest_routes_current_year) + "_lyr"
block_group_current_year_lyr = str(block_group_current_year) + "_lyr"
...
arcpy.MakeFeatureLayer_management(mile_routes_current_year, mile_routes_current_year_lyr)
arcpy.MakeFeatureLayer_management(closest_routes_current_year, closest_routes_current_year_lyr)
arcpy.MakeFeatureLayer_management(block_group_current_year, block_group_current_year_lyr)
...
# Run Stats
arcpy.AddMessage("Running stats...")
...
arcpy.AddMessage("Captured standard deviation ranges...")

dist_score_current_year = sd_score(dist_range, 0)  # Call sdScore function to get {SCORE:SD} for distance
acre_score_current_year = sd_score(acre_range, 1)  # Call sdScore function to get {SCORE:SD} for acreage
park_score_current_year = sd_score(park_range, 1)  # Call sdScore function to get {SCORE:SD} for parks
arcpy.AddMessage("Captured block scores...")
...
arcpy.AddMessage("Captured block group scores...")
...
with arcpy.da.UpdateCursor(blocks_current_year_lyr, cb_fields) as cursor:  # Update cursor for census block layer
    for row in cursor:  # For each row, populate fields from dictionary
        ...
        del row
del cursor
arcpy.AddMessage("Assigned block scores...")

# Census Block Groups
if run_bg == "YES":
    bg_list = []
    with arcpy.da.SearchCursor(block_group_current_year_lyr, "GEOID") as cursor:
        for row in cursor:
    ...
    for bg in bg_list:
        q1 = """ GEOID = """ + "\'" + str(bg) + "\'"
        arcpy.SelectLayerByAttribute_management(block_group_current_year_lyr, "NEW_SELECTION", q1)
        arcpy.SelectLayerByLocation_management(blocks_current_year_lyr, "WITHIN", block_group_current_year_lyr,
        ...
        bg_fields = ["POP_2016", "DSCORE_2016", "ASCORE_2016", "PSCORE_2016", "TOTAL_SCORE", "POP_WGT", "WGT_SCORE"]
        exp_2 = """ GEOID = """ + "\'" + str(bg) + "\'"
        with arcpy.da.UpdateCursor(block_group_current_year_lyr, bg_fields, where_clause=exp_2) as cursor:
            for row in cursor:
                row[0] = bg_pop_sum
                row[1] = dist_total
                row[2] = acre_total
                ...
                del row
        del cursor
arcpy.AddMessage("Assigned block group scores")
