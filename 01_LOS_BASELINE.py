...
Baseline 2013
Level of Service
Script calculates citywide block access scores from 2013 data.
06/15/16
Rob Siwiec
"""
...
block_group = arcpy.GetParameterAsText(5)
...
block_group_lyr = "BLOCKGROUP_2013_lyr"
...
etj_pop = []
with arcpy.da.SearchCursor(blocks_lyr, "POP") as cursor:
    for row in cursor:
...
dist_sd = get_sd(dist_list)  # Call getSD function to get {GEOID:SD} for distance
acre_sd = get_sd(acre_list)  # Call getSD function to get {GEOID:SD} for acreage
park_sd = get_sd(park_list)  # Call getSD function to get {GEOID:SD} for parks
arcpy.AddMessage("Captured standard deviations...")

dist_score = sd_score(dist_range, 0)  # Call sdScore function to get {SCORE:SD} for distance
acre_score = sd_score(acre_range, 1)  # Call sdScore function to get {SCORE:SD} for acreage
park_score = sd_score(park_range, 1)  # Call sdScore function to get {SCORE:SD} for parks
arcpy.AddMessage("Captured block scores...")

dist_blocks = block_score(dist_sd, dist_score, 0)  # Call blockScore function to get {GEOID:SCORE} for distance
acre_blocks = block_score(acre_sd, acre_score, 1)  # Call blockScore function to get {GEOID:SCORE} for acreage
park_blocks = block_score(park_sd, park_score, 1)  # Call blockScore function to get {GEOID:SCORE} for parks
arcpy.AddMessage("Captured block group scores...")

# Set Scores
cb_fields = ["GEOID10", "DSCORE_2013", "ASCORE_2013", "PSCORE_2013"]  # fields in census block layer
with arcpy.da.UpdateCursor(blocks_lyr, cb_fields) as cursor:  # Update cursor for census block layer
    for row in cursor:  # For each row, populate fields from dictionary
        for k, v in dist_blocks.items():  # Update field with SCORE from blockScore function
            if row[0] == k:
                row[1] = v
        for k, v in acre_blocks.items():  # Update field with SCORE from blockScore function
            if row[0] == k:
                row[2] = v
        for k, v in park_blocks.items():  # Update field with SCORE from blockScore function
            if row[0] == k:
                row[3] = v
        cursor.updateRow(row)
        del row
...
# Set Scores
cb_fields_1 = ["POP", "DSCORE_2013", "ASCORE_2013", "PSCORE_2013", "TOTAL_SCORE", "POP_WGT",
               "WGT_SCORE"]  # fields in census block layer
with arcpy.da.UpdateCursor(blocks_lyr, cb_fields_1) as cursor:  # Update cursor for census block layer
    for row in cursor:  # For each row, populate fields from dictionary
...
if run_bg == "YES":
    ...
    for bg in bg_list:
        ...
        arcpy.SelectLayerByLocation_management(blocks_lyr, "WITHIN", block_group_lyr, selection_type="NEW_SELECTION")
        ...
        exp2 = """ GEOID = """ + "\'" + str(bg) + "\'"
        with arcpy.da.UpdateCursor(block_group_lyr, bg_fields, where_clause=exp2) as cursor:
            for row in cursor:
                row[0] = bg_pop_sum
                row[1] = dist_total
                row[2] = acre_total
